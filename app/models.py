...
class TaskItem(Base):
    ...
    # 建立与进度的关系
    progresses = relationship("TaskProgress",
                              back_populates="item",
                              cascade="all, delete-orphan")
...
class TaskNotify(Base):
    ...
    stop_at = Column(DateTime(timezone=True), server_default=func.now())
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    run_mode = Column(Integer, nullable=False, server_default='0')
    run_code = Column(Integer, nullable=False)
    time_at = Column(Time)
    week_at = Column(Integer)
    ...
    # 建立与进度的关系
    progress = relationship("TaskProgress")
